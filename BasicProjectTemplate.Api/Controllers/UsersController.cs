...
using BasicProjectTemplate.Api.DTOs.User;
using BasicProjectTemplate.Application.UseCases.Users.Commands;
...
namespace BasicProjectTemplate.Api.Controllers;

public class UsersController(
    ISender _sender,
    IJwtProvider _jwtProvider) : BaseController
{
    [HttpGet("{id:guid}")]
    ...
    {
        var result = await _sender.Send(new RegisterUserInfoQuery());
        return Ok(result);
    }
    ...
    {
        User user = await _sender.Send(query, cancellationToken);
